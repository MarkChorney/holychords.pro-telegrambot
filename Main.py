...
# 2nd version || Create InlineKeyboard Button & MarkUp ||--------------
# ToFavorite = types.InlineKeyboardMarkup(row_width=1)
# button = types.InlineKeyboardButton("Save To Favorite",
#                               callback_data='Save To Favorite')
# ToFavorite.add(button)

# 3nd version || Create InlineKeyboard Button & MarkUp ||--------------

ToFavorite = types.InlineKeyboardMarkup(
    inline_keyboard=[
        [
            types.InlineKeyboardButton(text="Save To Favorite", callback_data='Save To Favorite')
        ]
    ]
)
# ---------------------------------------------------------------------
...
# --------------------------|| /favorite ||----------------------------
# ---------------------------------------------------------------------
@router.message(Command(commands=["favorite", 'db']))
...
# ----------------------------|| /new ||----------------------------
# ---------------------------------------------------------------------
@router.message(Command(commands=['new', 'n']))
...
# ----------------------------|| /popular ||----------------------------
# ---------------------------------------------------------------------
@router.message(Command(commands=['popular', 'top', 'chart', 'charts', 't']))
...
# --------------------------|| /help ||----------------------------
# ---------------------------------------------------------------------
@router.message(Command(commands=["help", "info", "support"]))
...
# --------------------------|| /search ||----------------------------
# ---------------------------------------------------------------------
@router.message(Command(commands=["search", 's', 'find']))
    ...
    if search_prompt == "":
        ...
        if len(required_tuple) == 0:
            ...
            text = 'Sorry, I couldn’t find, Try Google it 😞\n' \
                   'Google: [' + user_input + '](http://google.com/search?q=' + str(search_prompt) + ')'
...
async def process_callback_click(callback_query: types.CallbackQuery):
    Save_to_me_button = types.InlineKeyboardButton(text='Me', callback_data='Save_to_me')
    Save_to_group_button = types.InlineKeyboardButton(text='Group',
                                                callback_data='Save_to_group')
    Saved_button = types.InlineKeyboardButton(text='Saved', callback_data='Saved')

    where_to_safe = types.InlineKeyboardMarkup(
        inline_keyboard=[[Save_to_me_button, Save_to_group_button]])

    saved_markup = types.InlineKeyboardMarkup(
